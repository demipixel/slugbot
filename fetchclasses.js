...
function fetchTerm(term, cb) {
  ...
  request.post(
    ...
    { form: { action: 'results', 'binds[:term]': term, rec_dur: 50000 } },
    (err, http, body) => {
      const classNumbers = [];
      ...
      do {
        m = re.exec(body);
        if (m && classNumbers[classNumbers.length - 1] != m[1])
          classNumbers.push(m[1].replace(/&amp;/g, '&'));
      } while (m);
...
function fetchClasses(term, classNumbers, classes, cb) {
  ...
  const done = () => {
    ...
    if (doneCount % 200 == 0)
      console.log(
        `[Term ${term} class fetch] ${Math.round(
          (doneCount / classNumbers.length) * 100,
        )}% complete`,
      );
...
function parseClass(classBody) {
  ...
  do {
    m = re.exec(classBody);
    if (m)
      classData[convertToCamelCase(m[1])] = m[2]
        .toLowerCase()
        .replace(/<\/span>/g, '');
  } while (m);
  ...
  classData.status = classData.status
    ? (classData.status.match(/alt="(.+?)"/) || ['', 'Unknown'])[1]
    : 'Unknown';
  classData.credits = classData.credits
    ? parseInt((classData.credits.match(/\d+/) || [-1])[0])
    : 'Unknown';

  // Get name
  classData.fullName = classBody
    .match(/<h2 style="margin:0px;">([^]+?)<\/h2>/)
    .replace(/&nbsp;/g, ' ');
  classData.fullName = classData.fullName
    ? classData.fullName[1].trim().replace(/ +/g, ' ') || 'Unknown'
    : 'Cannot Find';
  classData.name = classData.fullName.split('-')[0];

  // Get other information from larger sections
  classData.description = (classBody.match(
    /Description[^]+?<div class="panel-body">([^]+?)<\/div>/,
  ) || ['', ''])[1];
  classData.requirements = (classBody.match(
    /Requirements[^]+?<div class="panel-body" >([^]+?)<\/div>/,
  ) || ['', ''])[1];
  classData.notes = (classBody.match(
    /Notes[^]+?<div class="panel-body" >([^]+?)<\/div>/,
  ) || ['', ''])[1];
  classData.meeting = {};
...
module.exports = function(term, cb) {
  fetchTerm(term, classes => {
    fs.writeFileSync(
      './classdata.json',
      JSON.stringify({ lastUpdated: Date.now(), classes: classes }),
    );
    if (cb) cb(classes);
  });
};
